...
class root extends mysqlconsultas{
    public function obtener_solicitudes($id_area){
        $qry = "SELECT s.id, s.fecha, s.hora, s.clave_solicitud, u.nombre
                ...
                WHERE s.estatus = 1 AND s.id_almacen = {$_SESSION['almacen']} AND u.id_area = '$id_area'
    ...
    public function obtener_materiales_por_agotarse($id_area){
        $qry = "SELECT p.id, p.nombre, c.cantidad FROM inv_productos p
                LEFT JOIN inv_almacen_producto c ON c.id_producto = p.id
                WHERE c.cantidad <= 10 and c.id_almacen = {$_SESSION['almacen']} AND p.id_area = '$id_area'";
        $res = $this->consulta($qry);
        return $res;
    }
    ...
    public function obtener_transferencias_en_curso($id_area){
        $qry = "SELECT s.id, s.codigo_transfer, c.nombre AS almacen_origen, a.nombre AS almacen_destino
                FROM inv_salida_transferencia s
                LEFT JOIN inv_almacen c ON c.id = s.id_almacen
                LEFT JOIN inv_almacen a ON a.id = s.id_almacen_destino
                LEFT JOIN inv_productos p ON p.id = s.id_producto
    ...
    public function productos_activos($id_area){
        $qry = "SELECT COUNT(p.id_estatus) as disponibles FROM inv_almacen_producto p
